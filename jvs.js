...
function displayQuestion() {
  const word = wordLibrary[currentIndex];
  const wordOrderLabel = document.getElementById("wordOrderLabel");
  const wordQuestion = document.getElementById("wordQuestion");
  const wordEntry = csvData.find(e => e.word === word);
  const ids = csvData.filter(item => item.word === word).map(item => item.id);

  // Prevent root word in options
  const isRoot = w => w !== word;

  // Fetch meaning
  const meanings = csvData
    .map(e => e.extra1)
    .filter((m, i, arr) => m && arr.indexOf(m) === i && isRoot(csvData[i].word));

  const correctMeaning = wordEntry?.extra1 || "";
  const meaningOptions = generateOptions(meanings, correctMeaning, 1);

  // Fetch synonyms
  const synonyms = new Set();
  const allWords = csvData.map(e => e.word);
  ids.forEach(id1 => {
    csvData.forEach(({ word: w2, id: id2 }) => {
      if (id1 === id2 && w2 !== word) synonyms.add(w2);
    });
  });
  const synonymOptions = generateOptions(allWords.filter(w => isRoot(w)), [...synonyms], 2);

  // Fetch antonyms
  const antonyms = new Set();
  ids.forEach(id1 => {
    csvData.forEach(({ word: w2, id: id2 }) => {
      if (id1 === -id2 && w2 !== word) antonyms.add(w2);
    });
  });
  const antonymOptions = generateOptions(allWords.filter(w => isRoot(w)), [...antonyms], 2);

  // Display root word & order
  wordOrderLabel.textContent = `Word ${currentIndex + 1}:`;
  wordQuestion.textContent = `Which one of the following is correct for "${word}"?`;

  // Display Meaning
  const meaningCard = document.getElementById("meaningquestionCard");
  const meaningDiv = document.getElementById("meaningWord");
  if (correctMeaning) {
    meaningCard.classList.remove("hidden");
    meaningDiv.innerHTML = generateChoiceHTML(meaningOptions, correctMeaning);
  } else {
  ...
  // Display Synonyms
  const synLabel = document.getElementById("synQLabel");
  const synDisplay = document.getElementById("synQDisplay");
  const synCard = document.getElementById("synonymquestionCard");
  if (synonyms.size > 0) {
    synCard.classList.remove("hidden");
    synLabel.textContent = "Which ones of these are correct Synonyms?";
    synDisplay.innerHTML = generateChoiceHTML(synonymOptions, [...synonyms]);
  } else {
    synCard.classList.add("hidden");
    synDisplay.innerHTML = "";
  }
  ...
  // Display Antonyms
  const antLabel = document.getElementById("antQLabel");
  const antDisplay = document.getElementById("antQDisplay");
  const antCard = document.getElementById("antonymquestionCard");
  if (antonyms.size > 0) {
    antCard.classList.remove("hidden");
    antLabel.textContent = "Which ones of these are correct Antonyms?";
    antDisplay.innerHTML = generateChoiceHTML(antonymOptions, [...antonyms]);
  } else {
    antCard.classList.add("hidden");
    antDisplay.innerHTML = "";
  }
...
function generateOptions(allOptions, correct, type) {
  const isMultiAnswer = Array.isArray(correct);
  const correctSet = new Set(isMultiAnswer ? correct : [correct]);
  const pool = allOptions.filter(opt => !correctSet.has(opt));

  const optionCount = randomOptionCount === 1
    ? Math.floor(Math.random() * (maxOptions - minOptions + 1)) + minOptions
    : minOptions;

  let correctCount = isMultiAnswer
    ? Math.ceil((correctPercent / 100) * optionCount)
    : 1;

  const finalCorrect = shuffleArray([...correctSet]).slice(0, correctCount);
  const incorrectCount = optionCount - finalCorrect.length;
  const finalIncorrect = shuffleArray(pool).slice(0, incorrectCount);

  return shuffleArray([...finalCorrect, ...finalIncorrect]);
}

// Generate HTML for options with click handlers
function generateChoiceHTML(options, correct) {
  const correctSet = new Set(Array.isArray(correct) ? correct : [correct]);
  return options.map(option => {
    const isCorrect = correctSet.has(option);
    return `<button class="optionBtn" onclick="handleChoice(this, ${isCorrect})">${option}</button>`;
  }).join(" ");
}

// Click handler
function handleChoice(btn, isCorrect) {
  btn.classList.add(isCorrect ? "correctChoice" : "wrongChoice");
}
